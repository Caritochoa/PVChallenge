...
import sqlite3
import os
import requests # I had to installed in the internal library.
import xml.etree.ElementTree as ET


def delete_database(): #Method to clear the data base before reload the Html.file.
    filelist = [ f for f in os.listdir('.') if f == 'categories_ht.db' ]
    for f in filelist:
        os.remove(f)

# Method for Data Base creation with the attributes in the Document.
def create_database():
...
def get_categories():# getting the categories from Ebay. Making the Api Request.
    request_headers = {
    ...
    root = ET.fromstring(r.text.encode('utf-8', 'strict'))
